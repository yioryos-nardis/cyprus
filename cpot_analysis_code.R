#####Replication Code for c-pot.org blog posts
...
#####Blog Post: How disaffected are Cypriots with Political Institutions?: EU Institutions#####
...
#Fuction calling 'Func' and calcuating weighted survey estimates for a given trust measure by group and subgroup (GCC/TCC) across time
...
dataInfo <- list(eu1, eu2, eu3, eu4, eu5, eu6, eu7, eu8, eu9, eu10, eu11, eu12, eu13, eu14) #Trust in EU
trust_eur <- applyFunc("trust_eu_gcc", "GCC", dataInfo)

dataInfo <- list(ep1, ep2, ep3, ep4, ep5, ep6, ep7, ep8, ep9, ep10, ep11, ep12, ep13, ep14) #Trust in EP
results <- applyFunc("trust_ep_gcc", "GCC", dataInfo)
trust_eur <- cbind(trust_eur, results)

dataInfo <- list(ec1, ec2, ec3, ec4, ec5, ec6, ec7, ec8, ec9, ec10, ec11, ec12, ec13, ec14) #Trust in EC
results <- applyFunc("trust_ec_gcc", "GCC", dataInfo)
trust_eur <- cbind(trust_eur, results)

dataInfo <- list(ecb1, ecb2, ecb3, ecb4, ecb5, ecb6, ecb7, ecb8, ecb9, ecb10, ecb11, ecb12, ecb13, ecb14) #Trust in ECB
results <- applyFunc("trust_ecb_gcc", "GCC", dataInfo)
trust_eur <- cbind(trust_eur, results)


#Trust in EU Institutions among TCC
dataInfo <- list(eu1, eu2, eu3, eu4, eu5, eu6, eu7, eu8, eu9, eu10, eu11, eu15, eu16, eu17) #Trust in EU
...
results <- rbind(results, NA, NA, NA, NA)
trust_eur <- cbind(trust_eur, results)


#Trust in EU Institutions among EU-15
dataInfo <- list(eu18, eu19, eu20, eu21, eu22, eu23, eu24, eu25, eu26, eu27, eu28, eu29, eu30, eu31) #Trust in EU
results <- applyFunc("trust_eu_eu15", "1", dataInfo)
trust_eur <- cbind(trust_eur, results)

dataInfo <- list(ep15, ep16, ep17, ep18, ep19, ep20, ep21, ep22, ep23, ep24, ep25, ep26, ep27, ep28) #Trust in EP
results <- applyFunc("trust_ep_eu15", "1", dataInfo)
trust_eur <- cbind(trust_eur, results)

dataInfo <- list(ec15, ec16, ec17, ec18, ec19, ec20, ec21, ec22, ec23, ec24, ec25, ec26, ec27, ec28) #Trust in EC
results <- applyFunc("trust_ec_eu15", "1", dataInfo)
trust_eur <- cbind(trust_eur, results)

dataInfo <- list(ecb15, ecb16, ecb17, ecb18, ecb19, ecb20, ecb21, ecb22, ecb23, ecb24, ecb25, ecb26, ecb27, ecb28) #Trust in ECB
results <- applyFunc("trust_ecb_eu15", "1", dataInfo)
trust_eur <- cbind(trust_eur, results)


#Trust in EU Institutions among EU-25
dataInfo <- list(eu32, eu33, eu34, eu35, eu36, eu37, eu38, eu39, eu40, eu41, eu42, eu43, eu44, eu45) #Trust in EU
results <- applyFunc("trust_eu_eu25", "1", dataInfo)
trust_eur <- cbind(trust_eur, results)

dataInfo <- list(ep29, ep30, ep31, ep32, ep33, ep34, ep35, ep36, ep37, ep38, ep39, ep40, ep41, ep42) #Trust in EP
results <- applyFunc("trust_ep_eu25", "1", dataInfo)
trust_eur <- cbind(trust_eur, results)

dataInfo <- list(ec29, ec30, ec31, ec32, ec33, ec34, ec35, ec36, ec37, ec38, ec39, ec40, ec41, ec42) #Trust in EC
results <- applyFunc("trust_ec_eu25", "1", dataInfo)
trust_eur <- cbind(trust_eur, results)

dataInfo <- list(ecb29, ecb30, ecb31, ecb32, ecb33, ecb34, ecb35, ecb36, ecb37, ecb38, ecb39, ecb40, ecb41, ecb42) #Trust in ECB
results <- applyFunc("trust_ecb_eu25", "1", dataInfo)
trust_eur <- cbind(trust_eur, results)

trust_eur$year <- c(2004:2017)
...
trust_eur[, -17] <- lapply(trust_eur[, -17], mult_one)
trust_eur <- round(trust_eur, 1)
...
trust_eur$weight <- c(552213/709314, 552213/709314, 552213/709314, 596752/753853, 638900/782126, 638900/782126, 651400/794626, 660400/803626, 651400/794626, 660400/803626, 724084/867310, 724084/867310, 724084/867310, 741308/884534, 
...
trust_eur <- trust_eur %>% mutate_at(vars(-year, -group, -weight), funs(round(., 1)))
...
#Trust in EU institutions for Cyprus
...
plot_trust_eu_cyp <- plot_ly(trust_eur_cyp, y = ~trust_eu, x= ~year, color = ~group, type = "scatter",
                             mode = "lines+markers",
                             line = list(color = factor(trust_eur_cyp$group, labels = c("FFCF00","9702A7","00A876")), shape = "spline", width = 3), 
...
subplot_trust_eu_inst_cyp <- plotly::subplot(plot_trust_eu_cyp, plot_trust_ep_cyp, plot_trust_ec_cyp, plot_trust_ecb_cyp, 
                                             nrows = 2, margin = 0.12, shareX = FALSE, shareY = FALSE, 
                                             titleY = TRUE) %>% 
  layout(annotations = anot_trust_eur_inst_cyp) 

plot_trust_eu_cyp
plot_trust_ep_cyp
plot_trust_ec_cyp
plot_trust_ecb_cyp
subplot_trust_eu_inst_cyp



#Trust in EU institutions for EU-15 & EU-25
...
plot_trust_eu_eu <- plot_ly(trust_eur_eu, y = ~trust_eu, x= ~year, color = ~group, type = "scatter",
                             mode = "lines+markers",
                             line = list(color = factor(trust_eur_eu$group, labels = c("#009999","#9702A7","#FF7800")), shape = "spline", width = 3), 
...
plot_trust_ecb_eu <- plot_ly(trust_eur_eu, y = ~trust_ecb, x= ~year, color = ~group, type = "scatter",
                            mode = "lines+markers",
                            line = list(color = factor(trust_eur_eu$group, labels = c("#009999","#9702A7","#FF7800")), shape = "spline", width = 3), 
                            marker = list(color = factor(trust_eur_eu$group, labels = c("#009999","#9702A7","#FF7800")), size = 6),
                            legendgroup = ~group, showlegend = F) %>% 
